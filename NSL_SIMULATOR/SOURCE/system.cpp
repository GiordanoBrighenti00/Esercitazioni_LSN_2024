...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    double p_Boltz = exp(-_beta * _measurement(_index_tenergy));
    _particle(i).setspin(-1);
    double p_alt = exp(-_beta * _measurement(_index_tenergy));
    double p_cond = p_Boltz/(p_Boltz + p_alt);
    if(_rnd.Rannyu() < p_cond){
      _particle(i).setspin(1);
    }else{
      _particle(i).setspin(-1);
    }
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp("../OUTPUT/potential_energy.txt");
        ...
        index_property++;
        _vtail = 1.0 * 8.0 * M_PI * _rho * (  1/ ( 9 * pow(_r_cut,9) ) - 1 / ( 3 * pow(_r_cut,3) ) ); // TO BE FIXED IN EXERCISE 7
      } else if( property == "KINETIC_ENERGY" ){
        ofstream coutk("../OUTPUT/kinetic_energy.txt");
        ...
        ofstream coutt("../OUTPUT/total_energy.txt");
        ...
        ofstream coutte("../OUTPUT/temperature.txt");
        ...
        ofstream coutpres("../OUTPUT/pressure.txt");
        coutpres << "#     BLOCK:   ACTUAL_P:     P_AVE:       ERROR:" << endl;
        coutpres.close();
        _nprop++;
        _measure_pressure = true;
        _index_pressure = index_property;
        index_property++;
        _ptail = 3 * 1.0 * 32.0 * M_PI * _rho * (  1/ ( 9 * pow(_r_cut,9) ) - 1 / ( 6 * pow(_r_cut,3) ) ); // TO BE FIXED IN EXERCISE 7
      } else if( property == "GOFR" ){
        ofstream coutgr("../OUTPUT/gofr.txt");
        ...
        ofstream coutpr("../OUTPUT/magnetization.txt");
        ...
        ofstream coutpr("../OUTPUT/specific_heat.txt");
        ...
        ofstream coutpr("../OUTPUT/susceptibility.txt");
...
void System :: finalize(){
  ...
  // Added part: saving the final uncertainty through different simulations
  
  coutf.open("../OUTPUT/final_measures.txt",ios::app);
  coutf 
    << '\t' << _nsteps       
    << '\t' << _global_av(_index_penergy)/double(_nblocks)
    << '\t' << this->error(_global_av(_index_penergy), _global_av2(_index_penergy), _nblocks)
    << endl;
...
void System :: measure(){ // Measure properties
  ...
  double virial=0.0;
  double pressure_temp=0.0;  // Riga aggiunta: va eventualmente cancellata se il codice non funziona correttamente!!!
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(dr < _r_cut){
          ...
          if(_measure_pressure) pressure_temp += 1.0/pow(dr,12) - 0.5/pow(dr,6); // PRESSURE: Riga aggiunta da cancellare!!!
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure and _measure_temp){ // Parte da cancellare!!!
    _measurement(_index_pressure) = _ptail + _rho * _measurement(_index_temp) + (48.0/(3.0*_volume)) * pressure_temp / double(_npart);
  }
  ...
  if(_measure_cv and _measure_tenergy){
    spec_heat += _measurement(_index_tenergy) * _measurement(_index_tenergy);
    _measurement(_index_cv) = spec_heat;
  }

  // SUSCEPTIBILITY /////
  if(_measure_chi){
    double spin2 = 0.0;  //Somma degli spin
    for(int i=0; i<_npart; i++) spin2 += _particle(i).getspin();
    _measurement(_index_chi) = _beta * (spin2 * spin2);
...
void System :: averages(int blk){
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open("../OUTPUT/potential_energy.txt",ios::app);
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open("../OUTPUT/kinetic_energy.txt",ios::app);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open("../OUTPUT/total_energy.txt",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open("../OUTPUT/temperature.txt",ios::app);
  ...
  if(_measure_pressure){  //Parte aggiunta: va cancellata se il codice non funziona!!!
    coutf.open("../OUTPUT/pressure.txt",ios::app);
  ...
  // MAGNETIZATION ///
  if(_measure_magnet){  //Parte aggiunta: va cancellata se il codice non funziona!!!
    coutf.open("../OUTPUT/magnetization.txt",ios::app);
  ...
  // SPECIFIC HEAT ///
  if(_measure_cv){  //Parte aggiunta: va cancellata se il codice non funziona!!!
    coutf.open("../OUTPUT/specific_heat.txt",ios::app);
    //(*) Devo sottrarre il quadarto della media in fase dell'energia, che non potevo calcolare in SYS.measure()
    average =  _average(_index_cv) - _average(_index_tenergy)*_average(_index_tenergy); //(*)
    //(**) Devo calcolare la media globale del calore specifico, mediata tra tutti i blocchi: < < H ^ 2 > - < H > < H > >
    sum_average = _global_av(_index_cv) - _global_av2(_index_tenergy); //(**)
    sum_ave2 = _global_av2(_index_cv) - _global_av2(_index_tenergy);
    coutf << "\t" << blk
          << "\t" << (_beta*_beta)*average*(_npart)
          << "\t" << sum_average*(_beta*_beta)*(_npart)/double(blk)
          << "\t" << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
  }
  
  // SUSCEPTIBILITY ///
  if(_measure_chi){  //Parte aggiunta: va cancellata se il codice non funziona!!!
    coutf.open("../OUTPUT/susceptibility.txt",ios::app);
    average = _average(_index_chi);
    sum_average = _global_av(_index_chi) ;
    sum_ave2 = _global_av2(_index_chi);
    coutf << "\t" << blk
          << "\t" << average / (_npart)
          << "\t" << sum_average/ (double(blk) * _npart)
          << "\t" << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
  }
